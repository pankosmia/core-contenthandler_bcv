...
function TsvExport() {
  ...
  const tsvExportOneBook = async (bookCode) => {
    ...
    if (!bookTsvResponse.ok) {
      enqueueSnackbar(
        `${doI18n("pages:core-contenthandler_bcv:could_not_fetch", i18nRef.current)} ${bookCode}`,
        { variant: "error" },
      );
      return false;
    }
    let blob = new Blob([bookTsvResponse.text], {
      type: "text/tab-separated-values",
    });
    saveAs(blob, `${bookCode}.tsv`);
    enqueueSnackbar(
  ...
  return (
    <Box>
      ...
      <PanDialog
        titleLabel={doI18n(
          "pages:core-contenthandler_bcv:export_as_tsv",
          i18nRef.current,
        )}
        closeFn={() => handleClose()}
        isOpen={open}
        ...
        <DialogContent sx={{ mt: 1 }} style={{ overflow: "hidden" }}>
          <Box sx={{ maxHeight: "269px" }}>
            <DialogContentText>
              <Typography>
                {doI18n(
                  "pages:core-contenthandler_bcv:pick_one_or_more_books_export",
                  i18nRef.current,
                )}
              </Typography>
        ...
        <PanDialogActions
          ...
          actionFn={async () => {
            if (!selectedBooks || selectedBooks.length === 0) {
              enqueueSnackbar(
                doI18n(
                  "pages:core-contenthandler_bcv:no_books_selected",
                  i18nRef.current,
                ),
                { variant: "warning" },
              );
              ...
              for (let b of selectedBooks) {
                await tsvExportOneBook(b);
              }
            }
            handleCloseCreate();
          }}
          actionLabel={doI18n(
            "pages:core-contenthandler_bcv:export_label",
            i18nRef.current,
          )}
